...
<head>
	...
	<body>
        ...
        var option = {
            ...
            legend: {	//图表上方的类别显示           	
            	show:true,
            	data:['蓝色（blue）','绿色（green）','黄色（yellow）','未知（unknow）']
            },
            color:[
                   'blue',	//蓝色曲线颜色
                   'green',	//绿色曲线颜色
		...
		myChart.showLoading();	//数据加载完之前先显示一段简单的loading动画
		 ...
		 var blue_counts=[];		//蓝色车牌数组
		 var green_counts=[];		//绿色车牌数组
		 var yellow_counts=[];		//黄色车牌数组
		 var unknow_counts=[];		//未知车牌数组
         ...
         url : "record.action",	//请求发送到ShowInfoIndexServlet处
                 /* 一个固定的name */
         data : {name:"A0001"},		//请求内包含一个key为统一的name，value为A0001的参数；服务器接收到客户端请求时通过request.getParameter方法获取该参数值
         dataType : "json",		//返回数据形式为json
         success : function(result) {
        	 //请求成功时执行该函数内容，result即为服务器返回的json对象
	         if (result != null && result.length > 0) {
	                for(var i=0;i<result.length;i++){       
	                	blue_counts.push(result[i].blue_count);		//分别取出蓝色、绿色、黄色和未知颜色车牌等值并填入前面声明的各个颜色车牌相对应的数组
	                	green_counts.push(result[i].green_count);
	                	yellow_counts.push(result[i].yellow_count);
	                	unknow_counts.push(result[i].unknow_count)
