...
<BODY>
	...
	<form action="${url }" method="post">
		<input name="chepai" type="text" value="${chepai }"
			...
			name="pinpai" type="text" value="${pinpai }" placeholder="请输入适用车型："
			style="height: 28px"> <input name="xinghao" type="text"
			value="${xinghao }" placeholder="请输入车位类型：" style="height: 28px">
		<input name="yanse" type="text" value="${yanse }"
			placeholder="请输入付费类型：" style="height: 28px"> 
			...
			<c:forEach items="${list}" var="bean">
				<TR>
					...
					<TD align="center" data-field="id"><a
						href="${url2 }update3.do?id=${bean.id }" class="layui-btn">查看详情</a>
						&nbsp; <c:if test="${bean.status=='车位空闲'}">
							<a href="jiluadd.do?carid=${bean.id }"
								class="layui-btn layui-btn-normal">车辆车位</a>
