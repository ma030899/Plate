...
<BODY>
	...
	<form action="${url }" method="post">
		<input name="chepai" type="text" value="${chepai }"
			placeholder="请输入车位号：" style="height: 28px"> <input
			type="submit" value="查询" class="layui-btn layui-btn-xs"
			style="height: 28px" />
	</form>
		...
		<TABLE cellSpacing=0 cellPadding=0 width="100%" align=center border=1
			...
			<c:forEach items="${list}" var="bean">
				...
				<TR>
					...
					<TD align="center" data-field="id"><a
						href="${url2 }update3.do?id=${bean.id }" class="layui-btn">查看</a>
						&nbsp; <c:if test="${bean.status=='车为空闲'}">
							<a href="${url2 }delete2.do?id=${bean.id }"
								onclick="return confirm('确定要维修吗?'); "
								class="layui-btn layui-btn-normal">维修</a>
      &nbsp;
 	  </c:if> <c:if test="${bean.status=='维修中'}">
							<a href="${url2 }delete3.do?id=${bean.id }"
								onclick="return confirm('确定要设置吗?'); "
								class="layui-btn layui-btn-danger">设置车位故障</a>
      &nbsp;
 	  </c:if></TD>
				</TR>
			</c:forEach>
