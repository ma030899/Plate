							...
							<TR>
								...
								<TD vAlign=top>
									...
									<form action="${url }" method="post">
										适用:<input name="pinpai" type="text" value="${pinpai }" /> &nbsp;&nbsp;  车位型号:<%-- <input
											name="xinghao" type="text" value="${xinghao }" /> --%>
						<select name="xinghao" >
                          	<option value="" selected="selected">所有型号</option>
                          	<option value="人防工程车位">人防工程车位</option>
                          	<option value="单产权车位">单产权车位</option>
                          	<option value="公共车位" >公共车位</option>
                          	<option value="开发商所有车位" >开发商所有车位</option>
                          	<option value="地下车位" >地下车位</option>
                          	<option value="电动车车位" >电动车车位</option>
                          	
                          </select>
                           &nbsp;&nbsp; 付/免费:<input
											name="yanse" type="text" value="${yanse }" style="width:50px" /> <input
																		...
																		<c:forEach items="${list}" var="bean">
																			...
																			<tr>
																				...
																				<td><a href="carview.do?id=${bean.id }">查看详情</a>
																					&nbsp;&nbsp;&nbsp; <c:if
																						test="${bean.status=='车位空闲'}">
																						<input
														type="button" class="btn btn-danger" value="预定车位"
														onClick="if(confirm('确认预定?')){window.location.href='yudingadd.do?carid=${bean.id }'}return false;"/>
																						<%-- <a href="yudingadd.do?carid=${bean.id }"
																							onclick="{if(confirm('确认预定?')){return true;}return false;}">预定车位</a> --%>
																					</c:if></td>
